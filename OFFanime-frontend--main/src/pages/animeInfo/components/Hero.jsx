...
function Hero({ anime, jikanInfo, tmdbInfo, tmdbLoading }) {
  ...
  const status =
    jikanInfo?.status || anime?.status || "Unknown";

  const studios =
    jikanInfo?.studios?.length
      ? jikanInfo.studios
      : anime?.studios || [];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-8 items-start">
              ...
              <div className="pt-2 max-w-[1050px]">
                ...
                {tmdbLoading ? (
                  ...
                  <h1
                    className="
                      ...
                      leading-[0.95]
                      text-white
                      drop-shadow-[0_5px_30px_rgba(0,0,0,0.9)]
                    "
                  >
                    {title}
                  </h1>
                ...
                <div className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-sm text-gray-300">
                  ...
                  {jikanInfo?.year && (
                    <span>{jikanInfo.year}</span>
                  )}

                  {jikanInfo?.duration && (
                    <span>{jikanInfo.duration}</span>
                  )}

                  {jikanInfo?.rating && (
                    <span>{jikanInfo.rating}</span>
                  )}

                  {Array.isArray(studios) &&
                    studios.length > 0 && (
                      <span>
                        {studios.slice(0, 3).join(", ")}
                      </span>
                    )}
                </div>
                ...
                {jikanInfo && (
                  <div className="mt-7 grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-[820px]">
                    ...
                    <Stat
                      label="MEMBERS"
                      value={formatNumber(jikanInfo.members)}
                    />
                  </div>
                )}
