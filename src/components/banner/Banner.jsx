    ...
    <section className="spotlight w-full h-full relative overflow-hidden bg-[#050505]">
      ...
      <div className="absolute inset-0 z-[5] flex items-end">
        <div className="w-full flex justify-between items-end px-[60px] pb-[85px] max-[1400px]:px-[40px] max-[1400px]:pb-[70px] max-md:px-5 max-md:pb-16">
          ...
          {/* BUTTONS (desktop) */}
          <div className="flex items-center gap-4 mb-5 max-md:hidden">
            <Link
              to={`/watch/${createAnimeSlug(
                item.title || item.name,
                item.id
              )}?ep=1`}
              className="h-[64px] px-9 rounded-2xl bg-white text-black text-[18px] font-semibold flex items-center gap-3 transition-all duration-300 hover:scale-105 hover:bg-gray-200 shadow-2xl"
            >
            ...
            {hasTrailer && (
              <button
                ...
                className="h-[64px] px-9 rounded-2xl bg-[#ffbade]/90 text-black text-[18px] font-semibold flex items-center gap-3 transition-all duration-300 hover:scale-105 hover:bg-[#ffbade] shadow-2xl"
              >
                <FontAwesomeIcon icon={faFilm} className="text-[14px]" />
                Watch Trailer
              </button>
            )}
            ...
            <Link
              ...
              className="h-[64px] px-9 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/10 text-white text-[18px] font-semibold flex items-center justify-center transition-all duration-300 hover:bg-white/20"
            >
              Details
            </Link>
          </div>
        ...
        {/* BUTTONS (mobile) — compact row pinned along the bottom of the hero. */}
        <div className="hidden max-md:flex items-center gap-2.5 absolute bottom-6 left-5 right-5 z-[6]">
          <Link
            to={`/watch/${createAnimeSlug(
              item.title || item.name,
              item.id
            )}?ep=1`}
            className="flex-1 h-[46px] rounded-xl bg-white text-black text-[14px] font-semibold flex items-center justify-center gap-2 active:scale-95 transition"
          >
          ...
          {hasTrailer && (
            <button
              type="button"
              onClick={onPlayTrailer}
              className="flex-1 h-[46px] rounded-xl bg-[#ffbade]/90 text-black text-[14px] font-semibold flex items-center justify-center gap-2 active:scale-95 transition"
            >
          ...
          <Link
            to={`/${createAnimeSlug(item.title || item.name, item.id)}`}
            className="flex-1 h-[46px] rounded-xl bg-white/10 backdrop-blur-xl border border-white/10 text-white text-[14px] font-semibold flex items-center justify-center active:scale-95 transition"
          >
            Details
          </Link>
        </div>
