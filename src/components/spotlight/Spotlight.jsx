...
import getAnimeTrailer from "../../utils/getAnimeTrailer.utils";
...
const Spotlight = ({ spotlights = [] }) => {
  ...
  // Trailer state — a single shared modal driven by whichever slide's button is
  // clicked. `trailers` maps spotlight id -> embed URL ("" = known-no-trailer,
  // undefined = not yet fetched). We prefetch the trailer for the ACTIVE slide.
  const [trailers, setTrailers] = useState({});
  const [openTrailer, setOpenTrailer] = useState(null); // { url, title } | null

  useEffect(() => {
    const item = spotlights[activeIndex];
    const malId = item?.malId;
    if (!malId || trailers[item.id] !== undefined) return;
    let alive = true;
    getAnimeTrailer(malId).then((url) => {
      if (alive) setTrailers((prev) => ({ ...prev, [item.id]: url }));
    });
    return () => {
      alive = false;
    };
  }, [activeIndex, spotlights, trailers]);

  const playTrailer = async (item) => {
    if (!item) return;
    let url = trailers[item.id];
    if (url === undefined) url = await getAnimeTrailer(item.malId);
    if (url) setOpenTrailer({ url, title: item.title || item.name });
  };
          ...
          {spotlights.map((item, index) => (
            <SwiperSlide className="relative h-full" key={item.id || index}>
              <Banner
                ...
                hasTrailer={Boolean(trailers[item.id])}
                onPlayTrailer={() => playTrailer(item)}
              />
            </SwiperSlide>
          ))}
        </Swiper>
