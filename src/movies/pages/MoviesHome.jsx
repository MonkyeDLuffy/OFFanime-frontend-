...
import MovieContinueWatching from "../components/MovieContinueWatching";
...
export default function MoviesHome() {
  ...
  return (
    <main className="w-full min-h-screen bg-[#0a0a0f] text-white">
      <Helmet>
        <title>{`Movies & TV - ${website_name}`}</title>
        <meta
          name="description"
          content={`Stream popular movies and TV shows in HD on ${website_name}. Browse trending titles, now playing, top rated films and series — free, no ads.`}
        />
        <meta
          name="keywords"
          content="watch movies free, stream tv shows, trending movies, popular tv, top rated films, free streaming"
        />
        <link rel="canonical" href="https://offanime.cc/movies" />
        <meta property="og:title" content={`Movies & TV - ${website_name}`} />
        <meta
          property="og:description"
          content={`Watch trending movies and TV shows in HD for free on ${website_name}.`}
        />
        <meta property="og:url" content="https://offanime.cc/movies" />
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-8 max-[575px]:px-3 flex flex-col gap-y-12">
        {/* Live-action Continue Watching — separate from anime; only shows
            movie/TV items the user has started. */}
        <MovieContinueWatching />

        {/* Netflix-style Top 10 Today — uses the trending list once it lands. */}
        {rows.trending.done && rows.trending.data.length > 0 && (
          <MovieTop10 items={rows.trending.data} />
        )}
